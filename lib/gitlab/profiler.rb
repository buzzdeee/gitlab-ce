...
module Gitlab
  module Profiler
    ...
    def self.profile(url, logger: nil, post_data: nil, user: nil, private_token: nil)
      ...
      headers[token_header_name(url)] = private_token if private_token
      ...
      app.get('/api/v4/users') if api_request?(url) || Rails.env.development?
    ...
    def self.token_header_name(url)
      if api_request?(url)
        'HTTP_PRIVATE_TOKEN'
      else
        'PRIVATE-TOKEN'
