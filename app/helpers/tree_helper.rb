module TreeHelper
  ...
  def flatten_tree(root_path, tree, first_call: true)
    return tree.flat_path.sub(/\A#{root_path}\//, '') if first_call && tree.flat_path.present?

    subtree = Gitlab::Git::Tree.where(@repository, @commit.id, tree.path)
    if subtree.count == 1 && subtree.first.dir?
      return tree_join(tree.name, flatten_tree(root_path, subtree.first, first_call: false))
    else
      return tree.name
    end
  end
