...
describe Gitlab::CycleAnalytics::StageSummary do
  ...
  subject { described_class.new(project, from: from, current_user: user).data }
  ...
  describe "#commits" do
    it "finds the number of commits created after the 'from date'" do
      create_commit("Test message", project, user, 'master')
      from # Use the current `Time.now` value
      create_commit("Test message", project, user, 'master')
    ...
    it "doesn't find commits from other projects" do
      from # Use the current `Time.now` value
      create_commit("Test message", create(:project, :repository), user, 'master')
    ...
    it "finds a large (> 100) snumber of commits if present" do
      from # Use the current `Time.now` value
      create_commit("Test message", project, user, 'master', count: 100)
