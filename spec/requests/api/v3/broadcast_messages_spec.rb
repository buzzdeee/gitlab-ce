...
describe API::V3::BroadcastMessages do
  ...
  describe 'GET /broadcast_messages' do
    it 'returns a 401 for anonymous users' do
      get v3_api('/broadcast_messages')

      expect(response).to have_http_status(401)
    end

    it 'returns a 403 for users' do
      get v3_api('/broadcast_messages', user)
    ...
    it 'returns an Array of BroadcastMessages for admins' do
      ...
      get v3_api('/broadcast_messages', admin)
  ...
  describe 'GET /broadcast_messages/:id' do
    ...
    it 'returns a 401 for anonymous users' do
      get v3_api("/broadcast_messages/#{message.id}")

      expect(response).to have_http_status(401)
    end

    it 'returns a 403 for users' do
      get v3_api("/broadcast_messages/#{message.id}", user)

      expect(response).to have_http_status(403)
    end

    it 'returns the specified message for admins' do
      get v3_api("/broadcast_messages/#{message.id}", admin)
  ...
  describe 'POST /broadcast_messages' do
    it 'returns a 401 for anonymous users' do
      post v3_api('/broadcast_messages'), attributes_for(:broadcast_message)

      expect(response).to have_http_status(401)
    end

    it 'returns a 403 for users' do
      post v3_api('/broadcast_messages', user), attributes_for(:broadcast_message)
    ...
    context 'as an admin' do
      it 'requires the `message` parameter' do
        ...
        post v3_api('/broadcast_messages', admin), attrs
      ...
      it 'defines sane default start and end times' do
        ...
        travel_to(time) do
          post v3_api('/broadcast_messages', admin), message: 'Test message'
  ...
  describe 'PUT /broadcast_messages/:id' do
    ...
    it 'returns a 401 for anonymous users' do
      put v3_api("/broadcast_messages/#{message.id}"),
    ...
    it 'returns a 403 for users' do
      put v3_api("/broadcast_messages/#{message.id}", user),
        attributes_for(:broadcast_message)
    ...
    context 'as an admin' do
      it 'accepts new background and foreground colors' do
        ...
        put v3_api("/broadcast_messages/#{message.id}", admin), attrs
