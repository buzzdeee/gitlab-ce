...
describe API::V3::V3::AccessRequests do
  ...
  shared_examples 'GET /:sources/:id/access_requests' do |source_type|
    context "with :sources == #{source_type.pluralize}" do
      it_behaves_like 'a 404 response when source is private' do
        let(:route) { get v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", stranger) }
      end
        ...
        %i[developer access_requester stranger].each do |type|
          context "as a #{type}" do
            it 'returns 403' do
              ...
              get v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", user)
      ...
      context 'when authenticated as a master/owner' do
        it 'returns access requesters' do
          get v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", master)
  ...
  shared_examples 'POST /:sources/:id/access_requests' do |source_type|
    context "with :sources == #{source_type.pluralize}" do
      it_behaves_like 'a 404 response when source is private' do
        let(:route) { post v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", stranger) }
      end
        ...
        %i[developer master].each do |type|
          context "as a #{type}" do
            it 'returns 403' do
              expect do
                user = public_send(type)
                post v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", user)
      ...
      context 'when authenticated as an access requester' do
        it 'returns 400' do
          expect do
            post v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", access_requester)
      ...
      context 'when authenticated as a stranger' do
        context "when access request is disabled for the #{source_type}" do
          ...
          it 'returns 403' do
            expect do
              post v3_api("/#{source_type.pluralize}/#{source.id}/access_requests", stranger)
  ...
  shared_examples 'PUT /:sources/:id/access_requests/:user_id/approve' do |source_type|
    context "with :sources == #{source_type.pluralize}" do
      it_behaves_like 'a 404 response when source is private' do
        let(:route) { put v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}/approve", stranger) }
      end
      ...
      context 'when authenticated as a non-master/owner' do
        %i[developer access_requester stranger].each do |type|
          context "as a #{type}" do
            it 'returns 403' do
              user = public_send(type)
              put v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}/approve", user)
      ...
      context 'when authenticated as a master/owner' do
        it 'returns 201' do
          expect do
            put v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}/approve", master),
        ...
        context 'user_id does not match an existing access requester' do
          it 'returns 404' do
            expect do
              put v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{stranger.id}/approve", master)
  ...
  shared_examples 'DELETE /:sources/:id/access_requests/:user_id' do |source_type|
    context "with :sources == #{source_type.pluralize}" do
      it_behaves_like 'a 404 response when source is private' do
        let(:route) { delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}", stranger) }
      end
      ...
      context 'when authenticated as a non-master/owner' do
        %i[developer stranger].each do |type|
          context "as a #{type}" do
            it 'returns 403' do
              user = public_send(type)
              delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}", user)
      ...
      context 'when authenticated as the access requester' do
        it 'deletes the access requester' do
          expect do
            delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}", access_requester)
      ...
      context 'when authenticated as a master/owner' do
        it 'deletes the access requester' do
          expect do
            delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{access_requester.id}", master)
        ...
        context 'user_id matches a member, not an access requester' do
          it 'returns 404' do
            expect do
              delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{developer.id}", master)
        ...
        context 'user_id does not match an existing access requester' do
          it 'returns 404' do
            expect do
              delete v3_api("/#{source_type.pluralize}/#{source.id}/access_requests/#{stranger.id}", master)
