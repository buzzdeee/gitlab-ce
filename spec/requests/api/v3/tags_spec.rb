...
describe API::V3::Tags do
  ...
  describe 'POST /projects/:id/repository/tags' do
    ...
    shared_examples_for 'repository new tag' do
      it 'creates a new tag' do
        post v3_api(route, current_user), tag_name: tag_name, ref: 'master'
    ...
    context 'when authenticated', 'as a master' do
      ...
      it 'returns 400 if tag name is invalid' do
        post v3_api(route, current_user), tag_name: 'new design', ref: 'master'
      ...
      it 'returns 400 if tag already exists' do
        post v3_api(route, current_user), tag_name: 'new_design1', ref: 'master'
      ...
      it 'returns 400 if ref name is invalid' do
        post v3_api(route, current_user), tag_name: 'new_design3', ref: 'foo'
      ...
      context 'lightweight tags with release notes' do
        it 'creates a new tag' do
          post v3_api(route, current_user), tag_name: tag_name, ref: 'master', release_description: 'Wow'
      ...
      context 'annotated tag' do
        it 'creates a new annotated tag' do
          ...
          post v3_api(route, current_user), tag_name: 'v7.1.0', ref: 'master', message: 'Release 7.1.0'
  ...
  describe 'DELETE /projects/:id/repository/tags/:tag_name' do
    ...
    shared_examples_for 'repository delete tag' do
      it 'deletes a tag' do
        delete v3_api(route, current_user)
      ...
      context 'when tag does not exist' do
        ...
        it_behaves_like '404 response' do
          let(:request) { delete v3_api(route, current_user) }
  ...
  describe 'POST /projects/:id/repository/tags/:tag_name/release' do
    ...
    shared_examples_for 'repository new release' do
      it 'creates description for existing git tag' do
        post v3_api(route, user), description: description
      ...
      context 'when tag does not exist' do
        ...
        it_behaves_like '404 response' do
          let(:request) { post v3_api(route, current_user), description: description }
    ...
    shared_examples_for 'repository update release' do
      context 'on tag with existing release' do
        ...
        it 'updates the release description' do
          put v3_api(route, current_user), description: new_description
      ...
      context 'when tag does not exist' do
        ...
        it_behaves_like '404 response' do
          let(:request) { put v3_api(route, current_user), description: new_description }
          let(:message) { 'Tag does not exist' }
        end
      end
