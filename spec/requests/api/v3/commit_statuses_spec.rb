    ...
    context 'ci commit exists' do
      ...
      context "reporter user" do
        ...
        context 'all commit statuses' do
          before do
            get v3_api(get_url, reporter), all: 1
          end
        ...
        context 'latest commit statuses for specific ref' do
          before do
            get v3_api(get_url, reporter), ref: 'develop'
          end
        ...
        context 'latest commit statues for specific name' do
          before do
            get v3_api(get_url, reporter), name: 'coverage'
          end
  ...
  describe 'POST /projects/:id/statuses/:sha' do
    ...
    context 'developer user' do
        ...
        context "for #{status}" do
          context 'uses only required parameters' do
            it 'creates commit status' do
              post v3_api(post_url, developer), state: status
      ...
      context 'transitions status from pending' do
        before do
          post v3_api(post_url, developer), state: 'pending'
        end

        %w[running success failed canceled].each do |status|
          it "to #{status}" do
            expect { post v3_api(post_url, developer), state: status }.not_to change { CommitStatus.count }
      ...
      context 'with all optional parameters' do
        context 'when creating a commit status' do
          subject do
            post v3_api(post_url, developer), {
              state: 'success',
              context: 'coverage',
              ref: 'master',
              description: 'test',
      ...
      context 'when status is invalid' do
        before do
          post v3_api(post_url, developer), state: 'invalid'
        end
      ...
      context 'when commit SHA is invalid' do
        ...
        before do
          post v3_api(post_url, developer), state: 'running'
        end
    ...
    context 'reporter user' do
      before do
        post v3_api(post_url, reporter), state: 'running'
      end
    ...
    context 'guest user' do
      before do
        post v3_api(post_url, guest), state: 'running'
      end
