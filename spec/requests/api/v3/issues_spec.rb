...
describe API::V3::V3::Issues, :mailer do
  ...
  describe "GET /issues" do
    ...
    context "when authenticated" do
      ...
      it 'returns an array of closed issues' do
        get v3_api('/issues', user), state: :closed
      ...
      it 'returns an array of opened issues' do
        get v3_api('/issues', user), state: :opened
      ...
      it 'returns an array of all issues' do
        get v3_api('/issues', user), state: :all
      ...
      it 'returns issues assigned to me' do
        ...
        get v3_api('/issues', user2), scope: 'assigned-to-me'
      ...
      it 'returns issues authored by the given author id' do
        ...
        get v3_api('/issues', user), author_id: user2.id, scope: 'all'
      ...
      it 'returns issues assigned to the given assignee id' do
        ...
        get v3_api('/issues', user), assignee_id: user2.id, scope: 'all'
      ...
      it 'returns issues authored by the given author id and assigned to the given assignee id' do
        ...
        get v3_api('/issues', user), author_id: user2.id, assignee_id: user2.id, scope: 'all'
      ...
      it 'returns issues matching given search string for title' do
        get v3_api("/issues", user), search: issue.title
      ...
      it 'returns issues matching given search string for description' do
        get v3_api("/issues", user), search: issue.description
      ...
      it 'returns an array of labeled issues' do
        get v3_api("/issues", user), labels: label.title
      ...
      it 'returns an array of labeled issues when all labels matches' do
        ...
        get v3_api("/issues", user), labels: "#{label.title},#{label_b.title},#{label_c.title}"
      ...
      it 'returns an empty array if no issue matches labels' do
        get v3_api('/issues', user), labels: 'foo,bar'

        expect_paginated_array_response(size: 0)
      end

      it 'returns an array of labeled issues matching given state' do
        get v3_api("/issues", user), labels: label.title, state: :opened
      ...
      it 'returns unlabeled issues for "No Label" label' do
        get v3_api("/issues", user), labels: 'No Label'
      ...
      it 'returns an array of issues matching state in milestone' do
        get v3_api("/issues?milestone=#{milestone.title}"\
                '&state=closed', user)
      ...
      it 'returns an array of issues found by iids' do
        get v3_api('/issues', user), iids: [closed_issue.iid]
      ...
      it 'returns an empty array if iid does not exist' do
        get v3_api("/issues", user), iids: [99999]
  ...
  describe "GET /groups/:id/issues" do
    ...
    it 'returns issues matching given search string for title' do
      get v3_api("#{base_url}?search=#{group_issue.title}", user)
    ...
    it 'returns issues matching given search string for description' do
      get v3_api("#{base_url}?search=#{group_issue.description}", user)
    ...
    it 'returns an array of labeled issues when all labels matches' do
      ...
      get v3_api("#{base_url}", user), labels: "#{group_label.title},#{label_b.title},#{label_c.title}"
    ...
    it 'returns an array of issues found by iids' do
      get v3_api(base_url, user), iids: [group_issue.iid]
    ...
    it 'returns an empty array if iid does not exist' do
      get v3_api(base_url, user), iids: [99999]
    ...
    it 'returns an array of issues matching state in milestone' do
      get v3_api("#{base_url}?milestone=#{group_milestone.title}"\
              '&state=closed', user)
  ...
  describe "GET /projects/:id/issues" do
    ...
    it 'returns an array of labeled issues when all labels matches' do
      ...
      get v3_api("#{base_url}/issues", user), labels: "#{label.title},#{label_b.title},#{label_c.title}"
    ...
    it 'returns issues matching given search string for title' do
      get v3_api("#{base_url}/issues?search=#{issue.title}", user)
    ...
    it 'returns issues matching given search string for description' do
      get v3_api("#{base_url}/issues?search=#{issue.description}", user)
    ...
    it 'returns an array of issues found by iids' do
      get v3_api("#{base_url}/issues", user), iids: [issue.iid]
    ...
    it 'returns an empty array if iid does not exist' do
      get v3_api("#{base_url}/issues", user), iids: [99999]

      expect_paginated_array_response(size: 0)
    end

    it 'returns an empty array if not all labels matches' do
      get v3_api("#{base_url}/issues?labels=#{label.title},foo", user)
    ...
    it 'returns an array of issues matching state in milestone' do
      get v3_api("#{base_url}/issues?milestone=#{milestone.title}&state=closed", user)
  ...
  describe "GET /projects/:id/issues/:issue_iid" do
    it 'exposes known attributes' do
      get v3_api("/projects/#{project.id}/issues/#{issue.iid}", user)
    ...
    context 'confidential issues' do
      it "returns 404 for non project members" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", non_member)

        expect(response).to have_http_status(404)
      end

      it "returns 404 for project members with guest role" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", guest)

        expect(response).to have_http_status(404)
      end

      it "returns confidential issue for project members" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", user)
      ...
      it "returns confidential issue for author" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", author)
      ...
      it "returns confidential issue for assignee" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", assignee)
      ...
      it "returns confidential issue for admin" do
        get v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", admin)
  ...
  describe "PUT /projects/:id/issues/:issue_iid to update only title" do
    it "updates a project issue" do
      put v3_api("/projects/#{project.id}/issues/#{issue.iid}", user),
        title: 'updated title'
      expect(response).to have_http_status(200)
    ...
    context 'confidential issues' do
      it "returns 403 for non project members" do
        put v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", non_member),
          title: 'updated title'
        expect(response).to have_http_status(403)
      end

      it "returns 403 for project members with guest role" do
        put v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", guest),
          title: 'updated title'
        expect(response).to have_http_status(403)
      end

      it "updates a confidential issue for project members" do
        put v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", user),
          title: 'updated title'
        expect(response).to have_http_status(200)
        expect(json_response['title']).to eq('updated title')
      end
      ...
      it "updates a confidential issue for author" do
        put v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", author),
          title: 'updated title'
        expect(response).to have_http_status(200)
        expect(json_response['title']).to eq('updated title')
      end
      ...
      it "updates a confidential issue for admin" do
        put v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}", admin),
          title: 'updated title'
        expect(response).to have_http_status(200)
        expect(json_response['title']).to eq('updated title')
      end
  ...
  describe 'PUT /projects/:id/issues/:issue_iid with spam filtering' do
    ...
    it "does not create a new project issue" do
      ...
      put v3_api("/projects/#{project.id}/issues/#{issue.iid}", user), params
  ...
  describe 'PUT /projects/:id/issues/:issue_iid to update labels' do
    ...
    it 'removes all labels' do
      put v3_api("/projects/#{project.id}/issues/#{issue.iid}", user), labels: ''
  ...
  describe "PUT /projects/:id/issues/:issue_iid to update state and label" do
    ...
    it 'reopens a project isssue' do
      put v3_api("/projects/#{project.id}/issues/#{closed_issue.iid}", user), state_event: 'reopen'
  ...
  describe 'PUT /projects/:id/issues/:issue_iid to update due date' do
    it 'creates a new project issue' do
      ...
      put v3_api("/projects/#{project.id}/issues/#{issue.iid}", user), due_date: due_date
  ...
  describe "DELETE /projects/:id/issues/:issue_iid" do
    it "rejects a non member from deleting an issue" do
      delete v3_api("/projects/#{project.id}/issues/#{issue.iid}", non_member)
      expect(response).to have_http_status(403)
    end

    it "rejects a developer from deleting an issue" do
      delete v3_api("/projects/#{project.id}/issues/#{issue.iid}", author)
      expect(response).to have_http_status(403)
    end
    ...
    context "when the user is project owner" do
      ...
      it "deletes the issue if an admin requests it" do
        delete v3_api("/projects/#{project.id}/issues/#{issue.iid}", owner)
    ...
    it 'returns 404 when using the issue ID instead of IID' do
      delete v3_api("/projects/#{project.id}/issues/#{issue.id}", user)
  ...
  describe '/projects/:id/issues/:issue_iid/move' do
    ...
    it 'moves an issue' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/move", user),
    ...
    it 'moves the issue to another namespace if I am admin' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/move", admin),
               to_project_id: target_project2.id
    ...
    context 'when source project does not exist' do
      it 'returns 404 when trying to move an issue' do
        post v3_api("/projects/123/issues/#{issue.iid}/move", user),
                 to_project_id: target_project.id
  ...
  describe 'POST :id/issues/:issue_iid/subscribe' do
    it 'subscribes to an issue' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/subscribe", user2)
    ...
    it 'returns 304 if already subscribed' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/subscribe", user)

      expect(response).to have_http_status(304)
    end

    it 'returns 404 if the issue is not found' do
      post v3_api("/projects/#{project.id}/issues/123/subscribe", user)

      expect(response).to have_http_status(404)
    end

    it 'returns 404 if the issue ID is used instead of the iid' do
      post v3_api("/projects/#{project.id}/issues/#{issue.id}/subscribe", user)

      expect(response).to have_http_status(404)
    end

    it 'returns 404 if the issue is confidential' do
      post v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}/subscribe", non_member)
  ...
  describe 'POST :id/issues/:issue_id/unsubscribe' do
    it 'unsubscribes from an issue' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/unsubscribe", user)
    ...
    it 'returns 304 if not subscribed' do
      post v3_api("/projects/#{project.id}/issues/#{issue.iid}/unsubscribe", user2)

      expect(response).to have_http_status(304)
    end

    it 'returns 404 if the issue is not found' do
      post v3_api("/projects/#{project.id}/issues/123/unsubscribe", user)

      expect(response).to have_http_status(404)
    end

    it 'returns 404 if using the issue ID instead of iid' do
      post v3_api("/projects/#{project.id}/issues/#{issue.id}/unsubscribe", user)

      expect(response).to have_http_status(404)
    end

    it 'returns 404 if the issue is confidential' do
      post v3_api("/projects/#{project.id}/issues/#{confidential_issue.iid}/unsubscribe", non_member)
  ...
  describe 'GET :id/issues/:issue_iid/closed_by' do
    ...
    it 'returns merge requests that will close issue on merge' do
      get v3_api("/projects/#{project.id}/issues/#{issue.iid}/closed_by", user)
    ...
    context 'when no merge requests will close issue' do
      it 'returns empty array' do
        get v3_api("/projects/#{project.id}/issues/#{closed_issue.iid}/closed_by", user)
    ...
    it "returns 404 when issue doesn't exists" do
      get v3_api("/projects/#{project.id}/issues/9999/closed_by", user)
  ...
  describe "GET /projects/:id/issues/:issue_iid/user_agent_detail" do
    ...
    it 'exposes known attributes' do
      get v3_api("/projects/#{project.id}/issues/#{issue.iid}/user_agent_detail", admin)
    ...
    it "returns unautorized for non-admin users" do
      get v3_api("/projects/#{project.id}/issues/#{issue.iid}/user_agent_detail", user)
