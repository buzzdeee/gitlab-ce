...
describe API::V3::V3::Projects do
  ...
  describe 'GET /projects' do
    shared_examples_for 'projects response' do
      it 'returns an array of projects' do
        get v3_api('/projects', current_user), filter
    ...
    shared_examples_for 'projects response without N + 1 queries' do
      it 'avoids N + 1 queries' do
        control_count = ActiveRecord::QueryRecorder.new do
          get v3_api('/projects', current_user)
        end.count
    ...
    context 'when authenticated as regular user' do
      ...
      it "includes statistics if requested" do
        get v3_api('/projects', user), statistics: true
      ...
      context 'and with owned=true' do
        it 'returns an array of projects the user owns' do
          get v3_api('/projects', user4), owned: true
      ...
      context 'and with starred=true' do
        ...
        it 'returns the starred projects viewable by the user' do
          get v3_api('/projects', user3), starred: true
      ...
      context 'and with all query parameters' do
        ...
        context 'including owned filter' do
          it 'returns only projects that satisfy all query parameters' do
            get v3_api('/projects', user), { visibility: 'public', owned: true, starred: true, search: 'gitlab' }
        ...
        context 'including membership filter' do
          ...
          it 'returns only projects that satisfy all query parameters' do
            get v3_api('/projects', user), { visibility: 'public', membership: true, starred: true, search: 'gitlab' }
  ...
  describe 'POST /projects' do
    ...
    it 'creates new project with name and path and returns 201' do
      expect { post v3_api('/projects', user), path: 'path-project-Foo', name: 'Foo Project' }
        .to change { Project.count }.by(1)
      expect(response).to have_http_status(201)
    ...
    context 'when a visibility level is restricted' do
      ...
      it 'does not allow a non-admin to use a restricted visibility level' do
        post v3_api('/projects', user), project_param
      ...
      it 'allows an admin to override restricted visibility settings' do
        post v3_api('/projects', admin), project_param
  ...
  describe 'GET /users/:user_id/projects/' do
    ...
    it 'returns error when user not found' do
      get v3_api('/users/9999/projects/')
    ...
    it 'returns projects filtered by user' do
      get v3_api("/users/#{user4.id}/projects/", user)
  ...
  describe 'POST /projects/user/:id' do
    ...
    it 'creates new project without path but with name and return 201' do
      expect { post v3_api("/projects/user/#{user.id}", admin), name: 'Foo Project' }.to change {Project.count}.by(1)
      expect(response).to have_http_status(201)
    ...
    it 'creates new project with name and path and returns 201' do
      expect { post v3_api("/projects/user/#{user.id}", admin), path: 'path-project-Foo', name: 'Foo Project' }
        .to change { Project.count }.by(1)
      expect(response).to have_http_status(201)
  ...
  describe 'GET /projects/:id' do
    ...
    context 'when authenticated' do
      ...
      it "includes statistics if requested" do
        get v3_api("/projects/#{project.id}", user), statistics: true
      ...
      it "does not include import_error if user cannot admin project" do
        get v3_api("/projects/#{project.id}", user3)
  ...
  describe 'POST /projects/:id/unstar' do
    context 'on a starred project' do
      ...
      it 'unstars the project' do
        expect { post v3_api("/projects/#{project.id}/unstar", user) }.to change { project.reload.star_count }.by(-1)
    ...
    context 'on an unstarred project' do
      it 'does not modify the star count' do
        expect { post v3_api("/projects/#{project.id}/unstar", user) }.not_to change { project.reload.star_count }
  ...
  describe 'POST /projects/:id/fork' do
    ...
    context 'when authenticated' do
      it 'forks if user has sufficient access to project' do
        post v3_api("/projects/#{project.id}/fork", user2)
      ...
      it 'forks if user is admin' do
        post v3_api("/projects/#{project.id}/fork", admin)
      ...
      it 'fails on missing project access for the project to fork' do
        ...
        post v3_api("/projects/#{project.id}/fork", new_user)
      ...
      it 'fails if forked project exists in the user namespace' do
        post v3_api("/projects/#{project.id}/fork", user)
      ...
      it 'fails if project to fork from does not exist' do
        post v3_api('/projects/424242/fork', user)
      ...
      it 'forks with explicit own user namespace id' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: user2.namespace.id
      ...
      it 'forks with explicit own user name as namespace' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: user2.username
      ...
      it 'forks to another user when admin' do
        post v3_api("/projects/#{project.id}/fork", admin), namespace: user2.username
      ...
      it 'fails if trying to fork to another user when not admin' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: admin.namespace.id

        expect(response).to have_http_status(404)
      end

      it 'fails if trying to fork to non-existent namespace' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: 42424242
      ...
      it 'forks to owned group' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: group2.name
      ...
      it 'fails to fork to not owned group' do
        post v3_api("/projects/#{project.id}/fork", user2), namespace: group.name

        expect(response).to have_http_status(404)
      end

      it 'forks to not owned group when admin' do
        post v3_api("/projects/#{project.id}/fork", admin), namespace: group.name
    ...
    context 'when unauthenticated' do
      it 'returns authentication error' do
        post v3_api("/projects/#{project.id}/fork")
  ...
  describe 'POST /projects/:id/housekeeping' do
    ...
    context 'when authenticated as owner' do
      it 'starts the housekeeping process' do
        ...
        post v3_api("/projects/#{project.id}/housekeeping", user)
    ...
    context 'when authenticated as developer' do
      ...
      it 'returns forbidden error' do
        post v3_api("/projects/#{project.id}/housekeeping", user3)
    ...
    context 'when unauthenticated' do
      it 'returns authentication error' do
        post v3_api("/projects/#{project.id}/housekeeping")
