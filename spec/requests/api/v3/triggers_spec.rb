...
describe API::V3::V3::Triggers do
  ...
  describe 'POST /projects/:project_id/trigger/pipeline' do
    ...
    context 'Handles errors' do
      it 'returns bad request if token is missing' do
        post v3_api("/projects/#{project.id}/trigger/pipeline"), ref: 'master'

        expect(response).to have_http_status(400)
      end

      it 'returns not found if project is not found' do
        post v3_api('/projects/0/trigger/pipeline'), options.merge(ref: 'master')
    ...
    context 'Have a commit' do
      ...
      it 'creates pipeline' do
        post v3_api("/projects/#{project.id}/trigger/pipeline"), options.merge(ref: 'master')
      ...
      it 'returns bad request with no pipeline created if there\'s no commit for that ref' do
        post v3_api("/projects/#{project.id}/trigger/pipeline"), options.merge(ref: 'other-branch')
      ...
      context 'Validates variables' do
        ...
        it 'validates variables to be a hash' do
          post v3_api("/projects/#{project.id}/trigger/pipeline"), options.merge(variables: 'value', ref: 'master')
        ...
        it 'validates variables needs to be a map of key-valued strings' do
          post v3_api("/projects/#{project.id}/trigger/pipeline"), options.merge(variables: { key: %w(1 2) }, ref: 'master')
        ...
        it 'creates trigger request with variables' do
          post v3_api("/projects/#{project.id}/trigger/pipeline"), options.merge(variables: variables, ref: 'master')
    ...
    context 'when triggering a pipeline from a trigger token' do
      it 'does not leak the presence of project when token is for different project' do
        post v3_api("/projects/#{project2.id}/ref/master/trigger/pipeline?token=#{trigger_token}"), { ref: 'refs/heads/other-branch' }
      ...
      it 'creates builds from the ref given in the URL, not in the body' do
        expect do
          post v3_api("/projects/#{project.id}/ref/master/trigger/pipeline?token=#{trigger_token}"), { ref: 'refs/heads/other-branch' }
      ...
      context 'when ref contains a dot' do
        it 'creates builds from the ref given in the URL, not in the body' do
          ...
          expect do
            post v3_api("/projects/#{project.id}/ref/v.1-branch/trigger/pipeline?token=#{trigger_token}"), { ref: 'refs/heads/other-branch' }
  ...
  describe 'GET /projects/:id/triggers/:trigger_id' do
    context 'authenticated user with valid permissions' do
      it 'returns trigger details' do
        get v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user)
      ...
      it 'responds with 404 Not Found if requesting non-existing trigger' do
        get v3_api("/projects/#{project.id}/triggers/-5", user)
    ...
    context 'authenticated user with invalid permissions' do
      it 'does not return triggers list' do
        get v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user2)
    ...
    context 'unauthenticated user' do
      it 'does not return triggers list' do
        get v3_api("/projects/#{project.id}/triggers/#{trigger.id}")
  ...
  describe 'PUT /projects/:id/triggers/:trigger_id' do
    context 'authenticated user with valid permissions' do
      ...
      it 'updates description' do
        put v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user),
    ...
    context 'authenticated user with invalid permissions' do
      it 'does not update trigger' do
        put v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user2)
    ...
    context 'unauthenticated user' do
      it 'does not update trigger' do
        put v3_api("/projects/#{project.id}/triggers/#{trigger.id}")
  ...
  describe 'POST /projects/:id/triggers/:trigger_id/take_ownership' do
    context 'authenticated user with valid permissions' do
      it 'updates owner' do
        ...
        post v3_api("/projects/#{project.id}/triggers/#{trigger.id}/take_ownership", user)
    ...
    context 'authenticated user with invalid permissions' do
      it 'does not update owner' do
        post v3_api("/projects/#{project.id}/triggers/#{trigger.id}/take_ownership", user2)
    ...
    context 'unauthenticated user' do
      it 'does not update owner' do
        post v3_api("/projects/#{project.id}/triggers/#{trigger.id}/take_ownership")
  ...
  describe 'DELETE /projects/:id/triggers/:trigger_id' do
    context 'authenticated user with valid permissions' do
      it 'deletes trigger' do
        expect do
          delete v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user)
      ...
      it 'responds with 404 Not Found if requesting non-existing trigger' do
        delete v3_api("/projects/#{project.id}/triggers/-5", user)
    ...
    context 'authenticated user with invalid permissions' do
      it 'does not delete trigger' do
        delete v3_api("/projects/#{project.id}/triggers/#{trigger.id}", user2)
    ...
    context 'unauthenticated user' do
      it 'does not delete trigger' do
        delete v3_api("/projects/#{project.id}/triggers/#{trigger.id}")
