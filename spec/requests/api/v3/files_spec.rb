...
describe API::V3::Files do
  ...
  describe "GET /projects/:id/repository/files/:file_path" do
    shared_examples_for 'repository files' do
      it 'returns file attributes as json' do
        get v3_api(route(file_path), current_user), params
      ...
      it 'returns raw file info' do
        ...
        get v3_api(url, current_user), params
      ...
      context 'when mandatory params are not given' do
        it_behaves_like '400 response' do
          let(:request) { get v3_api(route("any%2Ffile"), current_user) }
        end
      end
      ...
      context 'when file_path does not exist' do
        ...
        it_behaves_like '404 response' do
          let(:request) { get v3_api(route('app%2Fmodels%2Fapplication%2Erb'), current_user), params }
      ...
      context 'when repository is disabled' do
        ...
        it_behaves_like '403 response' do
          let(:request) { get v3_api(route(file_path), current_user), params }
        end
      end
    end
    ...
    context 'when unauthenticated', 'and project is private' do
      it_behaves_like '404 response' do
        let(:request) { get v3_api(route(file_path)), params }
    ...
    context 'when authenticated', 'as a guest' do
      it_behaves_like '403 response' do
        let(:request) { get v3_api(route(file_path), guest), params }
      end
    end
  end
  ...
  describe "GET /projects/:id/repository/files/:file_path/raw" do
    shared_examples_for 'repository raw files' do
      ...
      it 'returns file by commit sha' do
        ...
        get v3_api(route(file_path) + "/raw", current_user), params
  ...
  describe "POST /projects/:id/repository/files/:file_path" do
    ...
    it "creates a new file in project repo" do
      post v3_api(route(file_path), user), valid_params
    ...
    it "returns a 400 bad request if no mandatory params given" do
      post v3_api(route("any%2Etxt"), user)
    ...
    it "returns a 400 if editor fails to create file" do
      ...
      post v3_api(route("any%2Etxt"), user), valid_params
    ...
    context "when specifying an author" do
      it "creates a new file with the specified author" do
        ...
        post v3_api(route("new_file_with_author%2Etxt"), user), valid_params
    ...
    context 'when the repo is empty' do
      ...
      it "creates a new file in project repo" do
        post v3_api(route("newfile%2Erb"), user), valid_params
  ...
  describe "PUT /projects/:id/repository/files" do
    ...
    it "updates existing file in project repo" do
      put v3_api(route(file_path), user), valid_params
    ...
    it "returns a 400 bad request if update existing file with stale last commit id" do
      ...
      put v3_api(route(file_path), user), params_with_stale_id
    ...
    it "updates existing file in project repo with accepts correct last commit id" do
      ...
      put v3_api(route(file_path), user), params_with_correct_id

      expect(response).to have_http_status(200)
    end

    it "returns a 400 bad request if no params given" do
      put v3_api(route(file_path), user)
  ...
  describe "DELETE /projects/:id/repository/files" do
    ...
    it "deletes existing file in project repo" do
      delete v3_api(route(file_path), user), valid_params

      expect(response).to have_http_status(204)
    end

    it "returns a 400 bad request if no params given" do
      delete v3_api(route(file_path), user)
  ...
  describe "POST /projects/:id/repository/files with binary file" do
    ...
    before do
      post v3_api(route(file_path), user), put_params
    end

    it "remains unchanged" do
      get v3_api(route(file_path), user), get_params
