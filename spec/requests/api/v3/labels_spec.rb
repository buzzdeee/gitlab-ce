...
describe API::V3::V3::Labels do
  ...
  describe 'POST /projects/:id/labels' do
    it 'returns created label when all params' do
      post v3_api("/projects/#{project.id}/labels", user),
           name: 'Foo',
           color: '#FFAABB',
           description: 'test',
           priority: 2
    ...
    it 'returns a 400 bad request if name not given' do
      post v3_api("/projects/#{project.id}/labels", user), color: '#FFAABB'
      expect(response).to have_http_status(400)
    end

    it 'returns a 400 bad request if color not given' do
      post v3_api("/projects/#{project.id}/labels", user), name: 'Foobar'
      expect(response).to have_http_status(400)
    end
  ...
  describe 'PUT /projects/:id/labels' do
    it 'returns 200 if name and colors and description are changed' do
      put v3_api("/projects/#{project.id}/labels", user),
          name: 'label1',
          new_name: 'New Label',
          color: '#FFFFFF',
          description: 'test'
    ...
    it 'returns 400 if no label name given' do
      put v3_api("/projects/#{project.id}/labels", user), new_name: 'label2'
      expect(response).to have_http_status(400)
      expect(json_response['error']).to eq('name is missing')
    end

    it 'returns 400 if no new parameters given' do
      put v3_api("/projects/#{project.id}/labels", user), name: 'label1'
      expect(response).to have_http_status(400)
  ...
  describe "POST /projects/:id/labels/:label_id/subscribe" do
    context "when label_id is a label title" do
      it "subscribes to the label" do
        post v3_api("/projects/#{project.id}/labels/#{label1.title}/subscribe", user)
