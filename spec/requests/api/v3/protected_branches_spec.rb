...
describe API::V3::V3::ProtectedBranches do
  ...
  describe "GET /projects/:id/protected_branches" do
    ...
    shared_examples_for 'protected branches' do
      it 'returns the protected branches' do
        get v3_api(route, user), per_page: 100
    ...
    context 'when authenticated as a guest' do
      ...
      it_behaves_like '403 response' do
        let(:request) { get v3_api(route, user) }
      end
    end
  end
  ...
  describe 'POST /projects/:id/protected_branches' do
    ...
    context 'when authenticated as a master' do
      ...
      it 'protects a single branch' do
        post v3_api("/projects/#{project.id}/protected_branches", user), name: branch_name
      ...
      it 'protects a single branch and developers can push' do
        post v3_api("/projects/#{project.id}/protected_branches", user),
      ...
      it 'returns a 409 error if the same branch is protected twice' do
        post v3_api("/projects/#{project.id}/protected_branches", user), name: protected_name
  ...
  describe "DELETE /projects/:id/protected_branches/unprotect/:branch" do
    ...
    it "unprotects a single branch" do
      delete v3_api("/projects/#{project.id}/protected_branches/#{branch_name}", user)

      expect(response).to have_gitlab_http_status(204)
    end

    it "returns 404 if branch does not exist" do
      delete v3_api("/projects/#{project.id}/protected_branches/barfoo", user)
