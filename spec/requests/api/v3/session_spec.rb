  ...
  describe "POST /session" do
    context "when valid password" do
      it "returns private token" do
        post v3_api("/session"), email: user.email, password: '12345678'
      ...
      context 'with 2FA enabled' do
        it 'rejects sign in attempts' do
          ...
          post v3_api('/session'), email: user.email, password: user.password
    ...
    context 'when email has case-typo and password is valid' do
      it 'returns private token' do
        post v3_api('/session'), email: user.email.upcase, password: '12345678'
    ...
    context 'when login has case-typo and password is valid' do
      it 'returns private token' do
        post v3_api('/session'), login: user.username.upcase, password: '12345678'
        expect(response.status).to eq 201
    ...
    context "when invalid password" do
      it "returns authentication error" do
        post v3_api("/session"), email: user.email, password: '123'
        expect(response).to have_http_status(401)
    ...
    context "when empty password" do
      it "returns authentication error with email" do
        post v3_api("/session"), email: user.email

        expect(response).to have_http_status(400)
      end

      it "returns authentication error with username" do
        post v3_api("/session"), email: user.username
    ...
    context "when empty name" do
      it "returns authentication error" do
        post v3_api("/session"), password: user.password
    ...
    context "when user is blocked" do
      it "returns authentication error" do
        user.block
        post v3_api("/session"), email: user.username, password: user.password
