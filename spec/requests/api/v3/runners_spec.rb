...
describe API::V3::V3::Runners do
  ...
  describe 'GET /runners' do
    context 'authorized user' do
      ...
      it 'filters runners by scope' do
        get v3_api('/runners?scope=active', user)
      ...
      it 'avoids filtering if scope is invalid' do
        get v3_api('/runners?scope=unknown', user)
  ...
  describe 'GET /runners/all' do
    context 'authorized user' do
      ...
      it 'filters runners by scope' do
        get v3_api('/runners/all?scope=specific', admin)
      ...
      it 'avoids filtering if scope is invalid' do
        get v3_api('/runners?scope=unknown', admin)
        expect(response).to have_http_status(400)
      end
    end
  ...
  describe 'GET /runners/:id' do
    context 'admin user' do
      context 'when runner is shared' do
        it "returns runner's details" do
          get v3_api("/runners/#{shared_runner.id}", admin)
      ...
      context 'when runner is not shared' do
        it "returns runner's details" do
          get v3_api("/runners/#{specific_runner.id}", admin)
    ...
    context "runner project's administrative user" do
      context 'when runner is not shared' do
        it "returns runner's details" do
          get v3_api("/runners/#{specific_runner.id}", user)
      ...
      context 'when runner is shared' do
        it "returns runner's details" do
          get v3_api("/runners/#{shared_runner.id}", user)
    ...
    context 'other authorized user' do
      it "does not return runner's details" do
        get v3_api("/runners/#{specific_runner.id}", user2)
    ...
    context 'unauthorized user' do
      it "does not return runner's details" do
        get v3_api("/runners/#{specific_runner.id}")
  ...
  describe 'PUT /runners/:id' do
    context 'admin user' do
      ...
      def update_runner(id, user, args)
        put v3_api("/runners/#{id}", user), args
      end
    end
    ...
    context 'authorized user' do
      context 'when runner is shared' do
        it 'does not update runner' do
          put v3_api("/runners/#{shared_runner.id}", user), description: 'test'
      ...
      context 'when runner is not shared' do
        it 'does not update runner without access to it' do
          put v3_api("/runners/#{specific_runner.id}", user2), description: 'test'
        ...
        it 'updates runner with access to it' do
          description = specific_runner.description
          put v3_api("/runners/#{specific_runner.id}", admin), description: 'test'
          specific_runner.reload
    ...
    context 'unauthorized user' do
      it 'does not delete runner' do
        put v3_api("/runners/#{specific_runner.id}")
  ...
  describe 'GET /projects/:id/runners' do
    context 'authorized user with master privileges' do
      it "returns project's runners" do
        get v3_api("/projects/#{project.id}/runners", user)
    ...
    context 'authorized user without master privileges' do
      it "does not return project's runners" do
        get v3_api("/projects/#{project.id}/runners", user2)
    ...
    context 'unauthorized user' do
      it "does not return project's runners" do
        get v3_api("/projects/#{project.id}/runners")
  ...
  describe 'POST /projects/:id/runners' do
    context 'authorized user' do
      ...
      it 'enables specific runner' do
        expect do
          post v3_api("/projects/#{project.id}/runners", user), runner_id: specific_runner2.id
      ...
      it 'avoids changes when enabling already enabled runner' do
        expect do
          post v3_api("/projects/#{project.id}/runners", user), runner_id: specific_runner.id
      ...
      it 'does not enable shared runner' do
        post v3_api("/projects/#{project.id}/runners", user), runner_id: shared_runner.id
      ...
      context 'user is admin' do
        it 'enables any specific runner' do
          expect do
            post v3_api("/projects/#{project.id}/runners", admin), runner_id: unused_specific_runner.id
          end.to change { project.runners.count }.by(+1)
          expect(response).to have_http_status(201)
        end
      end
      ...
      context 'user is not admin' do
        it 'does not enable runner without access to' do
          post v3_api("/projects/#{project.id}/runners", user), runner_id: unused_specific_runner.id
      ...
      it 'raises an error when no runner_id param is provided' do
        post v3_api("/projects/#{project.id}/runners", admin)
    ...
    context 'authorized user without permissions' do
      it 'does not enable runner' do
        post v3_api("/projects/#{project.id}/runners", user2)
    ...
    context 'unauthorized user' do
      it 'does not enable runner' do
        post v3_api("/projects/#{project.id}/runners")
