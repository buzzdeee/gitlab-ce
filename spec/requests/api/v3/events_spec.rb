  ...
  describe 'GET /events' do
    ...
    context 'when authenticated' do
      it 'returns users events' do
        get v3_api('/events?action=closed&target_type=issue&after=2016-12-1&before=2016-12-31', user)
  ...
  describe 'GET /users/:id/events' do
    context "as a user that cannot see the event's project" do
      it 'returns no events' do
        get v3_api("/users/#{user.id}/events", other_user)
    ...
    context "as a user that can see the event's project" do
      it 'accepts a username' do
        get v3_api("/users/#{user.username}/events", user)
      ...
      context 'when the list of events includes push events' do
        ...
        before do
          get v3_api("/users/#{user.id}/events?action=pushed", user)
        end
      ...
      context 'when there are multiple events from different projects' do
        ...
        it 'accepts filter parameters' do
          get v3_api("/users/#{user.id}/events?action=closed&target_type=issue&after=2016-12-1&before=2016-12-31", user)
  ...
  describe 'GET /projects/:id/events' do
    context 'when unauthenticated ' do
      it 'returns 404 for private project' do
        get v3_api("/projects/#{private_project.id}/events")
      ...
      it 'returns 200 status for a public project' do
        ...
        get v3_api("/projects/#{public_project.id}/events")
    ...
    context 'when not permitted to read' do
      it 'returns 404' do
        get v3_api("/projects/#{private_project.id}/events", non_member)
    ...
    context 'when authenticated' do
      it 'returns project events' do
        get v3_api("/projects/#{private_project.id}/events?action=closed&target_type=issue&after=2016-12-1&before=2016-12-31", user)
      ...
      it 'returns 404 if project does not exist' do
        get v3_api("/projects/1234/events", user)
    ...
    context 'when exists some events' do
      ...
      it 'avoids N+1 queries' do
        control_count = ActiveRecord::QueryRecorder.new do
          get v3_api("/projects/#{private_project.id}/events", user), target_type: :merge_request
        end.count
